...
%      min(size(T),R*ones(1,N)) using a truncated MLSVD. If one or more
%      initial factor matrices are provided compression is skipped. On
...
%      @cpd3_sgsd|cpdnn_nlsb|cpds_minf|cpds_nls|false] to choose the
%      algorithm used to refine the solution after decompression, if
%      applicable. By default, options.Refinement is equal to
%      options.Algorithm. The structure options.RefinementOptions will be
%      passed to the chosen refinement method. Parameters not set in
%      options.RefinementOptions will be copied from
%      options.AlgorithmOptions where possible.
%
%   Further options are:
%
...
size_tens = size(T);
N = ndims(T);
if iscell(R), U = R; R = max(cellfun('size',U,2)); end
...
if ~exist('U','var') && R > 1
    ...
    ratio = prod(size_core)/numel(T);
    if ischar(options.Compression) && strcmpi(options.Compression,'auto')
        if ratio < 0.5
            options.Compression = true;
            print('is mlsvd (compression ratio %.6g < 0.5)... ',ratio);
        else
...
if options.Compression
    Tunc = T;
    [V,T] = mlsvd(T,size_core);
    output.Compression.ratio = ratio;
    output.Compression.relerr = frob(tmprod(T,V,1:N)-Tunc)/frob(Tunc);
    output.Compression.Name = 'truncated MLSVD';
...
output.Initialization.relerr = frob(cpdgen(U)-T)/frob(T);
...
output.Algorithm.relerr = frob(cpdgen(U)-T)/frob(T);
...
elseif xsfunc(options.Refinement)
    ...
    output.Refinement.relerr = frob(cpdgen(U)-T)/frob(T);
