...
if ~isfield(options,'MaxR'), options.MaxR = numel(T)/max(size(T)); end
if ~isfield(options,'MinR'), options.MinR = 1; end
if ~isfield(options,'MaxRelErr'), options.MaxRelErr = 1e-1; end
...
lb = max(cell2mat(cellfun( ...
         @(s)[sqrt(flipud(cumsum(flipud(s(:).^2)))).'/norm(sv{1}) ...
              zeros(1,Rmax+1-length(s))],sv(:),'UniformOutput',false)));
lb = lb(2:end);

% Determine the range of rank-one terms to test.
R = find(lb(options.MinR:end)<=options.MaxRelErr,1,'first')+options.MinR-1;
...
for r = 1:length(R)
    ...
    if isfield(output,'Refinement') && ...
       isfield(output.Refinement,'fval')
        relerr(r) = sqrt(2*output.Refinement.fval(end))/norm(sv{1});
    elseif isfield(output,'Algorithm') && ...
           isfield(output.Algorithm,'fval')
        relerr(r) = sqrt(2*output.Algorithm.fval(end))/norm(sv{1});
    elseif isfield(output,'fval')
        relerr(r) = sqrt(2*output.fval(end))/norm(sv{1});
    else
        relerr(r) = frob(T-cpdgen(U))/norm(sv{1});
    end
...
logx = options.XMultiplier*[1;R(:)];
logy = log10([lb(1);relerr(:)]);
...
function Lcurve(update)
    ...
    hold off;
    ylim([min(options.MinRelErr/(10^0.5),min(relerr)) ...
          max(options.MaxRelErr*10^0.5,lb(1))]);
    xlim([1 R(r)]);
    ylabel('frob(T-cpdgen(U))/frob(T)'); xlabel('R');
    xt = get(gca,'XTick'); set(gca,'XTick',xt(mod(xt,1) == 0));
    if update
        legend(['CPD error (trying R = ' int2str(R(r+1)) '...)'], ...
               'Lower bound on error','Location','SW');
    else
        legend(['L-curve corner at R = ' int2str(R(opt))],'CPD error', ...
               'Lower bound on error','Location','SW');
    end
