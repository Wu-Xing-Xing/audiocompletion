function [alpha,output] = ls_mt(f,g,zk,pk,fk,gk,options)
%LS_MT Strong Wolfe line search by More-Thuente.
%   [alpha,output] = ls_mt(f,g,zk,pk,fk,gk) attempts to find a point along
%   the line zk+alpha*pk that satisfies the strong Wolfe conditions for the
%   function f and its gradient g. In other words, a step alpha is computed
%   so that both
%
%      f(zk+alpha*pk) <= f(zk)+c1*alpha*real(gk'*pk)         (suff. decr.)
%      abs(real(g(zk+alpha*pk)'*pk)) <= c2*abs(real(gk'*pk)) (curv. cond.)
%
%   where pk is a descent direction and 0 < c1 < c2 < 1, are satisfied. The
%   input variables fk and gk should equal f(zk) and g(zk), respectively.
%   If gk is empty, it is evaluated as gk = g(zk). The variable zk may be a
%   scalar, vector, matrix, tensor or even a cell array of tensors and its
%   contents may be real or complex. The variables pk and gk may have the
%   same structure as zk, but may also be given in vectorized form.
%
...
%      
%      output.dkp1    - The directional derivative real(gkp1'*pk).
%      output.fevals  - The amount of function/gradient calls.
%      output.fkp1    - The value of the function f at zk+alpha*pk.
%      output.gkp1    - The vectorized output of the function g at
%                       zk+alpha*pk.
...
%      output.zpk1    - The variable zk+alpha*pk.
%
%   ls_mt(f,g,zk,pk,fk,gk,options) may be used to set the following
%   options:
%
...
dgk = real(gk'*pk); % Initially: g = dgk. In the loop: g = dgstp.
ftol = options.c1;
gtol = options.c2;
xtol = options.TolX;
...
if dgk > 0
    error('ls_mt:gk', ...
          'Gradient at zk is not a descent direction.');
end
if ftol < 0
    error('ls_mt:c1', ...
...
while true
    ...
    zstp = deserialize(zk+stp*pk,dim);
    fstp = f(zstp);
    if ~isa(g,'function_handle') && isempty(g)
